...
async function analyzeTicketWithAI(ticketData) {
    ...
    try {
        const prompt = `أنت موظف في قولدن تيكت. العميل تواصل عبر الشات بوت واختار خيارات معينة.

بيانات البلاغ (مفصولة بفاصلة):
${ticketData.subject || ''}

هيكل الخيارات:
1. شراء تذكرة:
   - قبل الشراء → العميل يريد شراء تذكرة لفعالية معينة (الاسم بعد الفاصلة)
   - بعد الشراء → العميل اشترى وعنده مشكلة (فعالية إنتهت/قادمة/خارج السعودية)

2. بيع تذكرة:
   - قبل البيع → استفسار عن البيع
   - بعد البيع → مشكلة بعد البيع

اكتب جملة واحدة واضحة تشرح للموظف ماذا يحتاج العميل بالضبط.
